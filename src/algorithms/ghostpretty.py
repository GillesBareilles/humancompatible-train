...
def StochasticGhost(net: torch.nn.Module,
          data: torch.utils.data.Dataset,
          constraints: Iterable[FairnessConstraint],
          geomp, maxiter,max_runtime=np.inf, stepsize_rule = 'inv_iter', zeta=0.5, gamma0 = 0.1, rho=0.8, lamb=0.5, beta=10., tau=1.,seed=42):
    ...
    max_sample_size = max([len(c.group_sets) for c in constraints])
    ...
    for iteration in range(0, maxiter):
        ...
        for j, subp_batch_size in enumerate(mbatches):
            ...
            samples_c = [c.sample_dataset(N=subp_batch_size) for c in constraints]
            if j == 1:
